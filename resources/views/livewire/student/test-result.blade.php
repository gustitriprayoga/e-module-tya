    ...
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="bg-white border-2 border-slate-100 rounded-[32px] p-8 text-center relative overflow-hidden">
                ...
                <div class="text-6xl font-black {{ $preTestScore < 70 ? 'text-slate-400' : 'text-slate-700' }} mb-4">
                    {{ number_format($preTestScore ?? 0, 0) }}
                </div>
            </div>
        ...
        @if ($preTestScore !== null)
                ...
                <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div class="flex-1 text-center md:text-left">
                        ...
                        <p class="text-slate-400 max-w-md">
                            Calculates the actual improvement relative to maximum possible improvement.
                        </p>
                    </div>
            ...
            <div x-show="showDetails" x-transition:enter="transition ease-out duration-300"
                ...
                class="bg-white border-2 border-slate-100 rounded-[32px] p-8 shadow-lg shadow-slate-200/50 mb-12"
                x-cloak>

                <h3 class="text-2xl font-black text-slate-900 mb-8 border-b border-slate-100 pb-4">
                    Research Analytics Details
                </h3>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div>
                        <h4 class="text-sm font-black text-slate-400 uppercase tracking-widest mb-6">
                            1. N-Gain Calculation
                        </h4>
                        <div
                            class="bg-slate-50 p-6 rounded-2xl border border-slate-200 font-mono text-sm overflow-x-auto">
                            <p class="text-slate-500 mb-4 border-b border-slate-200 pb-2">
                            ...
                            <div class="text-lg text-slate-800 leading-relaxed">
                                @if ($nGainCategory === 'Maksimal (Perfect)')
                                    <div
                                        ...
                                        <strong>Perfect Score Maintained.</strong>
                                        Both Pre-Test and Post-Test = 100.
                                    </div>
                                @elseif ($preTestScore == 100 && $postTestScore < 100)
                                    <div class="text-red-600 bg-red-50 p-4 rounded-xl border border-red-100 text-sm">
                                        ...
                                        Pre-Test is already 100. Result is a Decrease.
                                    </div>
                                @else
                                    ...
                                    @if ($nGainCategory === 'Tetap (No Change)')
                                        ...
                                        <div
                                            ...
                                            No change: Pre-Test and Post-Test scores are equal.
                                        </div>
                                    @elseif ($isDecrease)
                                        = <strong
                                        ...
                                        <div
                                            ...
                                            Negative Gain: Post-Test score is lower than Pre-Test.
                                        </div>
                                    @else
                                        = <strong
                    ...
                    {{-- Kolom Kanan: Matrix dengan 5 kategori lengkap --}}
                    <div>
                        <h4 class="text-sm font-black text-slate-400 uppercase tracking-widest mb-6">
                            2. N-Gain Criteria Matrix (Hake, 1999)
                        </h4>
                        <div class="space-y-3">
                            ...
                            <div
                                class="flex items-center justify-between p-4 rounded-2xl border-2
                                {{ $isPerfect ? 'border-emerald-500 bg-emerald-50' : 'border-slate-100 bg-white opacity-50' }}">
                                <div>
                            ...
                            <div
                                class="flex items-center justify-between p-4 rounded-2xl border-2
                                {{ $isHigh ? 'border-green-500 bg-green-50' : 'border-slate-100 bg-white opacity-50' }}">
                                <div>
                            ...
                            <div
                                class="flex items-center justify-between p-4 rounded-2xl border-2
                                {{ $isMedium ? 'border-amber-500 bg-amber-50' : 'border-slate-100 bg-white opacity-50' }}">
                                <div>
                                    <div class="font-bold {{ $isMedium ? 'text-amber-700' : 'text-slate-700' }}">Medium
                                        (Sedang)</div>
                                    <div class="text-sm {{ $isMedium ? 'text-amber-600' : 'text-slate-500' }}">0.30
                                        &le; Score &lt; 0.70</div>
                                </div>
                                @if ($isMedium)
                                    <div
                            ...
                            {{-- Low — HANYA 0 < score < 0.3, bukan <= 0 --}}
                            @php $isLow = ($nGainScore > 0 && $nGainScore < 0.3 && !$isDecrease); @endphp
                            <div
                                class="flex items-center justify-between p-4 rounded-2xl border-2
                                {{ $isLow ? 'border-orange-500 bg-orange-50' : 'border-slate-100 bg-white opacity-50' }}">
                                <div>
                                    <div class="font-bold {{ $isLow ? 'text-orange-700' : 'text-slate-700' }}">Low
                                        (Rendah)</div>
                                    <div class="text-sm {{ $isLow ? 'text-orange-600' : 'text-slate-500' }}">0 &lt;
                                        Score &lt; 0.30</div>
                                </div>
                                @if ($isLow)
                                    <div
                            ...
                            <div
                                class="flex items-center justify-between p-4 rounded-2xl border-2
                                {{ $isNoChange ? 'border-slate-500 bg-slate-100' : 'border-slate-100 bg-white opacity-50' }}">
                                <div>
                            ...
                            {{-- Decrease — score negatif --}}
                            @php $isDecreaseRow = ($isDecrease && $nGainCategory !== 'Maksimal (Perfect)'); @endphp
                            <div
                                class="flex items-center justify-between p-4 rounded-2xl border-2
                                {{ $isDecreaseRow ? 'border-red-500 bg-red-50' : 'border-slate-100 bg-white opacity-50' }}">
                                <div>
                                ...
                                @if ($isDecreaseRow)
                                    <div
                                        class="bg-red-500 text-white text-xs font-black px-3 py-1 rounded-full shrink-0">
                                        Your Result</div>
                                @endif
                            </div>
