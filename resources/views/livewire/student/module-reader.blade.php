<div class="min-h-screen w-full bg-[#FDFBF7] flex flex-col font-sans">
    ...
    {{-- Konten utama — TIDAK ada overflow-hidden di sini --}}
            ...
            <div wire:loading.remove wire:target="nextPage, prevPage">
                @if ($currentPage)
                            ...
                            @elseif($block['type'] === 'quiz')
                                <div class="bg-white rounded-3xl border border-slate-100 p-6 sm:p-8 shadow-sm">
                                    ...
                                    <div class="space-y-3">
                                        @foreach ($block['content']['options'] as $index => $opt)
                                            ...
                                            <button
                                                ...
                                                {{ $status === 'wrong' && $opt['is_correct'] && !isset($userAnswers[$block['id']]) ? '' : '' }}
                                                ...
                                                <span
                                                    class="shrink-0 w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black transition-colors
                                                    {{ $status === 'correct' && $opt['is_correct'] ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-brand-100 group-hover:text-brand-600' }}">
                                                    {{ chr(65 + $index) }}
                                                </span>
                                                <span
                                                    class="font-semibold text-sm sm:text-base leading-snug
                                                    {{ $status === 'correct' && $opt['is_correct'] ? 'text-green-800' : 'text-slate-700' }}">
                                                    {{ $opt['answer'] }}
                                                </span>
                                            </button>
                                        @endforeach
                                    ...
                                    @if (isset($userAnswers[$block['id']]))
                                        <div
                                            class="mt-5 p-4 rounded-2xl flex items-center gap-3 font-semibold text-sm
                                            {{ $userAnswers[$block['id']] === 'correct' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200' }}">
                    ...
                    <div class="mt-16 pt-6 border-t border-slate-200 flex justify-between items-center gap-4">
                        ...
                        @else
                            {{-- Tambahkan wire:click="finishModule" di sini --}}
    ...
    {{-- Tooltip Vocabulary (mobile-friendly, click-based) --}}
