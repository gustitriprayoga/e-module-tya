<div class="min-h-screen w-full bg-[#FDFBF7] flex flex-col font-sans relative overflow-x-hidden" x-data="moduleTracker">
    ...
    {{-- Navbar --}}
    <nav class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 w-full">
        <div
            class="max-w-4xl mx-auto px-4 sm:px-6 h-auto min-h-[56px] py-2 flex flex-col sm:flex-row items-center justify-center sm:justify-between gap-2 sm:gap-4 relative">

            <a href="{{ route('dashboard') }}"
                class="sm:absolute sm:left-4 sm:top-1/2 sm:-translate-y-1/2 text-slate-500 hover:text-slate-800 transition-colors p-2 rounded-xl hover:bg-slate-100 flex items-center gap-2 self-start sm:self-auto">
            ...
            @if ($totalPages > 0)
                <div class="w-full max-w-[200px] sm:max-w-xs flex flex-col items-center gap-1.5 mt-1 sm:mt-0">
                    ...
                    <div class="space-y-12">
                        @forelse($contentBlocks as $block)
                            ...
                            @if (in_array($block['type'], ['pbl_intro', 'reading_text', 'text']))
                                <div
                                    class="font-serif text-slate-800 leading-relaxed sm:leading-[1.8] text-lg sm:text-xl text-left sm:text-justify w-full overflow-hidden break-words
