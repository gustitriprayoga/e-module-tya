<div class="relative min-h-screen overflow-hidden pt-32 pb-20">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10" x-data="{ shown: false }" x-init="setTimeout(() => shown = true, 100)">
        ...
        <div class="text-center mb-16" x-show="shown" x-transition:enter="transition ease-out duration-700"
            ...
            <span
                class="px-4 py-1.5 rounded-full bg-accent-100 text-accent-700 border border-accent-200 font-bold text-sm mb-4 inline-block tracking-wide">LitFlow
                Performers</span>
        ...
        @php
            // Pisahkan data podium dan tabel
            $top1 = $rankings->get(0);
            $top2 = $rankings->get(1);
            $top3 = $rankings->get(2);
            $others = $rankings->skip(3);
        @endphp
        ...
        @if ($rankings->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end mb-16 px-4 md:px-0">
                ...
                <div x-show="shown" x-transition:enter="transition ease-out duration-700 delay-300"
                    ...
                    class="order-2 md:order-1 {{ !$top2 ? 'opacity-0' : '' }}">
                    @if ($top2)
                        <div
                            ...
                            <div
                                ...
                                {{ substr($top2->user->name, 0, 2) }}
                            </div>
                            <h4 class="font-extrabold text-slate-900 text-xl truncate" title="{{ $top2->user->name }}">
                                {{ explode(' ', $top2->user->name)[0] }}
                            </h4>
                            <p class="text-brand-600 font-black text-3xl mt-1">{{ $top2->top_wpm }} <span
                ...
                <div x-show="shown" x-transition:enter="transition ease-out duration-700 delay-500"
                    ...
                    @if ($top1)
                        <div
                            class="bg-gradient-to-br from-brand-600 to-accent-600 p-10 rounded-[32px] text-center shadow-2xl shadow-brand-500/50 h-[340px] flex flex-col justify-center relative md:scale-110 border-4 border-white/30 hover:-translate-y-2 transition-transform duration-300">
                            ...
                            <div class="relative z-10">
                                ...
                                <h4 class="font-extrabold text-white text-3xl mb-2 mt-4 truncate"
                                    title="{{ $top1->user->name }}">
                                    {{ explode(' ', $top1->user->name)[0] }}
                                </h4>
                                <p
                                    class="text-brand-100 font-bold mb-4 uppercase text-[10px] tracking-widest bg-black/20 inline-block px-3 py-1 rounded-full">
                                    Top Speed Reader</p>
                                <p class="text-white font-black text-5xl drop-shadow-md">{{ $top1->top_wpm }} <span
                ...
                <div x-show="shown" x-transition:enter="transition ease-out duration-700 delay-100"
                    x-transition:enter-start="opacity-0 translate-y-16" class="order-3 {{ !$top3 ? 'opacity-0' : '' }}">
                    @if ($top3)
                        <div
                            ...
                            <div
                                ...
                                {{ substr($top3->user->name, 0, 2) }}
                            </div>
                            <h4 class="font-extrabold text-slate-900 text-lg truncate" title="{{ $top3->user->name }}">
                                {{ explode(' ', $top3->user->name)[0] }}
                            </h4>
                            <p class="text-brand-600 font-black text-2xl mt-1">{{ $top3->top_wpm }} <span
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-slate-900/5 border-b border-slate-200/60">
                                <tr>
                                    <th
                                        class="px-8 py-6 text-xs font-extrabold text-slate-500 uppercase tracking-widest w-24">
                                        Rank</th>
                                    <th
                                        class="px-8 py-6 text-xs font-extrabold text-slate-500 uppercase tracking-widest">
                                        Student Info</th>
                                    <th
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach ($others as $index => $rank)
                                    <tr class="hover:bg-white/80 transition-colors duration-200 group">
                                        <td class="px-8 py-6 font-black text-slate-400 text-lg">#{{ $index + 4 }}
                                        </td>
                                        <td class="px-8 py-6">
                                            <div
                                            ...
                                            <div class="text-sm text-slate-500 font-medium mt-1">NIM:
                                                {{ $rank->user->nim_nip ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-8 py-6 text-right">
                                            <span
                                                class="px-4 py-2 bg-brand-50 text-brand-700 border border-brand-100 rounded-xl font-bold shadow-sm inline-block">
                                                {{ $rank->top_wpm }} WPM
                                            </span>
                                        </td>
                                    </tr>
        ...
        @else
            <div x-show="shown" x-transition:enter="transition ease-out duration-700 delay-300"
                ...
                <h3 class="text-2xl font-extrabold text-slate-900 mb-2">No Records Yet</h3>
                <p class="text-slate-500 font-medium">The leaderboard will update automatically once students start
                    completing modules.</p>
            </div>
        @endif
